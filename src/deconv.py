import numpy as np
from scipy.optimize import fming_cg, brent
from scipy.misc import imshow
import utils
# should look into that utils module

def steepest_descent(funct, X0, grad, maxiter = 100, verbose = False, project = None, alpha_0 = 1.0, gtol = 1e-5, **args):
    def func_one(X, dir_):
        def inner(alpha):
            if project is None:
        ...
        def print_info (iter, objective, gnorm):
            print "not implemented print info"
        ...
        while True:
            ...
            if iter >= maxiter:
                if verbose:
                    print "max nr of iterations reached"
                break
        return X
...
class ObjFunc(object):
    ...
    def optimize_latent(self, L0=None, method="gd", verbose=True, **args):
        ...
        if method == "cg":
            ...
            Xopt = steepest_descent(self._eval_XL, X0, self._eval_grad_XL, project=self.projectXL, verbose=verbose,
                                    **args)
    ...
    def optimize_psf(self, P0=None, method="gd", verbose=True, **args):
        ...
        if method == "cg":
            ...
            Xopt = steepest_descent(self._eval_XP, X0, self._eval_grad_XP, project=self.projectXP, verbose=verbose,
                                    **args)
